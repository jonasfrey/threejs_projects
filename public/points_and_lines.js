...
var o_camera = new THREE.PerspectiveCamera( 50, window.innerWidth / window.innerHeight, 0.1, 1000 );
//important for VR
o_camera.position.set( 0, 5, 2 );




var n_camera_movement_speed = 0.08;
...
const o_default_phong_material_white = new THREE.MeshPhongMaterial(
    {
        color: 0xffffff,
        side: THREE.DoubleSide,
        flatShading: true,
...
const o_default_phong_material_transparent = new THREE.MeshPhongMaterial(
    {
        color: 0xefd00f,
        emissive: 0xefd00f,
        side: THREE.DoubleSide,
...
///
// o_geometry = new THREE.PlaneGeometry( 20,20 );
// o_material = o_default_phong_material_white
// const o_mesh_ground = new THREE.Mesh( o_geometry, o_material );
// o_mesh_ground.rotation.x = - Math.PI / 2;
// o_mesh_ground.receiveShadow = true;
// o_scene.add( o_mesh_ground );
...
///
var point_a = new THREE.Vector3(3,6,9)
var point_b = new THREE.Vector3(2,3,1)


o_geometry = new THREE.BufferGeometry().setFromPoints( [
    point_a, 
    point_b
] );

const o_line_point_a_point_b = new THREE.Line( 
    o_geometry, 
    o_default_line_material
);
o_scene.add( o_line_point_a_point_b );

///
var richtungs_vektor = new THREE.Vector3(
    point_b.x - point_a.x,
    point_b.y - point_a.y,
    point_b.z - point_a.z,
)
var offset = {x:0, y:0, z:0, universal_factor:0}
const fold = gui.addFolder('offset')
fold.add(offset, 'x', -4, 4).name("x").listen()
fold.add(offset, 'y', -4, 4).name("y").listen()
fold.add(offset, 'z', -4, 4).name("z").listen()
fold.add(offset, 'universal_factor', -4, 4).name("universal_factor").listen()
...
o_geometry = new THREE.BufferGeometry().setFromPoints( [
    ...
    // richtungs_vektor 
    new THREE.Vector3(
        richtungs_vektor.x * -111  ,
        richtungs_vektor.y * -111 + point_b.y,
        richtungs_vektor.z * -111 ,
    )
] );
o_material = o_default_line_material.clone()
o_material.color = new THREE.Color( 0xff0f0f )      
var o_line = new THREE.Line( 
    o_geometry, 
    o_material
    );
o_scene.add( o_line );
...
o_geometry = new THREE.BufferGeometry().setFromPoints( [
    ...
    new THREE.Vector3(
        richtungs_vektor.x * 111,
        richtungs_vektor.y * 111,
        richtungs_vektor.z * 111,
    )
] );

o_material = o_default_line_material.clone()
o_material.color = new THREE.Color( "orange" )  
var o_line = new THREE.Line( 
    o_geometry, 
    o_material
    );
o_line.f_render_function = function(){
    ...
    this.geometry = new THREE.BufferGeometry().setFromPoints( [
        new THREE.Vector3(
            0 + offset.universal_factor * richtungs_vektor.x,
            0 + offset.universal_factor * richtungs_vektor.y, 
            0 + offset.universal_factor * richtungs_vektor.z
        ),
        // richtungs_vektor 
        new THREE.Vector3(
            richtungs_vektor.x * -1 * 44 + offset.universal_factor * richtungs_vektor.x,
            richtungs_vektor.y * -1 * 44 + offset.universal_factor * richtungs_vektor.y,
            richtungs_vektor.z * -1 * 44 + offset.universal_factor * richtungs_vektor.z,
        )
    ] );
    console.log('asdf')
}
...
var b_x_durch_a_x = point_b.x / point_a.x
var b_y_durch_a_y = point_b.y / point_a.y
var b_z_durch_a_z = point_b.z / point_a.z
...
for(var i = 0; i < 100; i +=0.1){
    ...
    var factor = i
    var o_point_a_multiplied_by_factor = new THREE.Vector3(
        i,
        i,
        i
    )
    
    o_geometry = new THREE.SphereGeometry( 0.1, 16, 16 );

    o_material = new THREE.MeshBasicMaterial( { color: 0x00ff00 } );
    var o_mesh_sphere = new THREE.Mesh( o_geometry, o_material );
    o_mesh_sphere.position.x = o_point_a_multiplied_by_factor.x;
    o_mesh_sphere.position.y = o_point_a_multiplied_by_factor.y;
    o_mesh_sphere.position.z = o_point_a_multiplied_by_factor.z;

    var o_mesh_clone = o_mesh_sphere.clone()
    o_mesh_clone.position.x = (point_b.x * b_x_durch_a_x * 2 * i);
    o_mesh_clone.position.y = (point_b.y * b_y_durch_a_y * 2 * i);
    o_mesh_clone.position.z = (point_b.z * b_z_durch_a_z * 2 * i);

    // o_scene.add( o_mesh_sphere );
    o_scene.add( o_mesh_clone );
