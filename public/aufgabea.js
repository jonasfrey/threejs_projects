...
o_geometry = new THREE.PlaneGeometry( 4, 4 );
o_material = o_default_phong_material_transparent
const o_mesh_ebene = new THREE.Mesh( o_geometry, o_material );
o_mesh_ebene.rotation.x = - Math.PI / 2;

o_mesh_ebene.receiveShadow = true;
o_mesh_ebene.castShadow = true; 
o_scene.add( o_mesh_ebene );

const gui_folder_ebene = gui.addFolder('Ebene')
gui_folder_ebene.add(o_mesh_ebene.rotation, 'x', 0, Math.PI * 2).name("rotation.x").listen()
gui_folder_ebene.add(o_mesh_ebene.rotation, 'y', 0, Math.PI * 2).name("rotation.y").listen()
gui_folder_ebene.add(o_mesh_ebene.rotation, 'z', 0, Math.PI * 2).name("rotation.z").listen()
gui_folder_ebene.add(o_mesh_ebene.position, 'x', 0, 4).name("position.x").listen()
gui_folder_ebene.add(o_mesh_ebene.position, 'y', 0, 4).name("position.y").listen()
gui_folder_ebene.add(o_mesh_ebene.position, 'z', 0, 4).name("position.z").listen()
gui_folder_ebene.open()

var point_a = new THREE.Vector3(-1, 0, 4)
var point_b = new THREE.Vector3(-1, 0, 4)

var point_origin = new THREE.Vector3(0,0,0)
var stuetz_vektor = new THREE.Vector3(
    point_a.x - point_origin.x,
    point_a.y - point_origin.y,
    point_a.z - point_origin.z,
)
